...
%
% This calls the mex function sparseblockmex. A version compatible with your 
% operating system must be present for sparseblock to work.    
%
%
%
%See also UNSPARSIFY
...
SpX = sparseblockmex(real(X),Ir,Jc, nrows ); %sparseblockmex is a mex function

if any(imag(X(:))~=0)
   SpX  = SpX + 1i*sparseblockmex(imag(X),Ir,Jc, nrows );
end
