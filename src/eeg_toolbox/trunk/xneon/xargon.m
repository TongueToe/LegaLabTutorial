...
% Christopher Kovach 2016
    ...
    properties
        ...
        queue = 'NEUROSURGERY';  %Queue to which the request will be send: 'UI','UI-HM' or 'all.q'
        argon_matlab_bin = 'matlab'; %location of matlab binary used to run the script
        use_matlab_version = '';
        argon_jobs_path = '~/xargon_submit/jobs'; %Path to the jobs folder as seen on the argon server.
...
%         finished = false; 
    ...
    properties (SetAccess = private, Hidden = true)
       ...
       avail_profiles = {'std_mem',2.25,56,64
        ...
        curr_argonpath = '~/argon/xneon_submit';   %Path to network folder shared from argon.     
        curr_profile = 'high_mem';
...
%                     warndlg(sprintf('The folder\n\n    %s\n\nhas not been recently scanned.\n\nIs scanpath.sh running on the server?',fullfile(npath,'jobs')))
            ...
            me.path.remote= [me.argon_jobs_path,'/',me.uuid];
            ...
            if ~deferred
                for k = 1:length(me.datafiles)
                    if ~isempty(me.datafiles(k).orig) && ~exist(me.datafiles(k).local,'file')
                        ...
                        if isunix
                            system(sprintf('cp %s %s',me.datafiles(k).orig,me.datafiles(k).local));
                        else
...
%                 error('distributed parallel not implemented yet')
        ...
        function isscanned = checkscan(me)
           ...
           scf = dir(fullfile(me.argonpath,'jobs','scanned'));
