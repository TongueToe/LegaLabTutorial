...
for sbi = 1:length(trees)
    ...
    try
        sid =  regexp(dats.branch(end).entry(1).parsed(1).subjectnumbermandatory,'\d\d\d','match','once');
    ...
    for k = 1:length(dats.branch)
        ...
        if ~isfield(entries,'parsed') || all(cellfun(@isempty,{entries.parsed}))
            continue
        end
        d = entries(1).parsed;
        ...
        if ~isempty(regexp(labwikitxt,'#REDIRECT'))
            continue
        end
