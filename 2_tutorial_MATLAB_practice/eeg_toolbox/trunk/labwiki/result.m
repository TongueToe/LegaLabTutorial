...
%                          pos = pos+[0 -1]*w;
                     case 'Submit'
                         ui= uicontrol('style','pushbutton','units','normalized','position',[pos+ [.3,0], .3,w*1.5],'string',fields{k},'HorizontalAlignment','left','callback',@(varargin)me.write);
...
%             elc = [resp.Contact_Number];
            ...
            if ~isempty(me.guih) && ishandle(me.guih) && ishandle(me.handles.Subject)
                ...
                if iscell(hlbl)
                    set(me.handles.Contacts,'string',[{'Contacts'},hlbl]);                    
                else
        ...
        function newresp_callback(me,fig,handles,fields,varargin)
            ...
            delete(fig)
             me.query_params=struct('action','askargs','conditions','Category:Response Type');
              ...
              if ~isempty(me.guih) && ishandle(me.guih)
               set(me.handles.Response_Type,'value',find(strcmp(str.Label,me.available_response_types)));
              end         
